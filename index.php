...
<html lang="en">
    ...
    <body>
        ...
        &nbsp;<a href="index2.php" title="Top">cheatsheet 2.0</a><br><br><br>
        <div class="fixed">
        ...
        <div class="row full-width">
            <div class="large-4 columns code-column">
                <h4>
                    <a name="artisan" href="#mystuff">My Stuff</a>
                    <a href="" title="My Laravel Shortcuts">
                        <i class="icon-file-text"></i>
                    </a>
                </h4>
                <pre class="prettyprint lang-php">
// SHIT YOU GOOGLE LIKE EVERY DAY DUSTIN
// Make all the things: 
php artisan make:model Thing -mcr
// Make Model, Migrations, and Controller
php artisan make:model --migration --controller test
// Example of Migration up
public function up() {
    Schema::create('sms_alert', function (Blueprint $table) {
        $table->bigIncrements('id');
        $table->string('sms_number', 10);
        $table->string('message');
        $table->timestamps();
    });
}
// Directly Modify the table without a change(), this will FOR SURE change COL data type. 
Schema::table('contents', function (Blueprint $table) {
    DB::statement('ALTER TABLE contents MODIFY COLUMN body longtext');
});
// Limit index size 
$table->string('name', 100);
// Migrate specific paths (focus the migration to a set)
php artisan migrate --path=/app/database/migrations/nominate
// Rollback 1 step
...
Schema::table('contents', function (Blueprint $table) {
    $table->longText('body')->change();
    $table->longText('section_2_body')->change();
    $table->longText('section_3_body')->change();
});
...
// Example foreign key
public function up() {
    Schema::create('example_table_FK_for_above', function (Blueprint $table){
        $table->bigIncrements('id');
        $table->string('file_name');
        $table->string('store_item_id');
        $table->integer('sort_order');
        $table->foreign('store_item_id')->references('id')->on('example_table');
        $table->binary('image_content')->nullable();
        $table->dateTime('created_at')->default(\Carbon\Carbon::now());
        $table->dateTime('updated_at')->default(\Carbon\Carbon::now());
    });
}
...
php artisan routes
// List all registered routes Laravel 5+
